...
# save the full list to csv
assignee_year_topic_full = doc_topics.groupby(["assignee_id", "year"])["topic_id"].apply(list).reset_index()
assignee_year_topic_full.to_csv("assignee_year_topic_full_list.csv", index=False)

# drop assignees that only show up one year
...
# merge with trajectory and save
trajectory = assignee_trajectory[["assignee_id", "year", "distance_t-3", "distance_t-5"]]
full_list = pd.read_csv("assignee_year_topic_full_list.csv")
full_list = pd.merge(left=full_list, right=diver_assignee_year, on=["assignee_id", "year"], how="left")
full_list = pd.merge(left=full_list, right=trajectory, on=["assignee_id", "year"], how="left")
full_list = full_list[["assignee_id", "year", "n_patent", "topic_id",
                       "n_topics", "nuni_topics", "distance_t-3", "distance_t-5"]]
full_list = full_list.sort_values(by=["assignee_id", "year"]).reset_index(drop=True)
full_list.to_csv("assignee_diversification_trajectory.csv", index=False)
